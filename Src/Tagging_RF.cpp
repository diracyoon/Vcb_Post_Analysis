...
Tagging_RF::Tagging_RF(const TString &a_era, const TString &a_mode, const TString &a_channel, const TString &a_extension) : samples(a_era, a_channel)
{
...
float Tagging_RF::Get_Tagging_RF(const TString &sample, const TString &syst, const int &n_jet, const float &ht)
{
...
float Tagging_RF::Get_Tagging_RF(const TString &sample, const TString &syst, const int &n_pv)
{
  ...
  int bin = ratio_c[sample_index][syst_index]->FindBin(n_pv);
  return ratio_c[sample_index][syst_index]->GetBinContent(bin);
} // float Tagging_RF::Get_Tagging_RF(const TString &syst_name, const int &n_pv)
...
void Tagging_RF::Combine()
{
  ...
  fin_mu = new TFile("Vcb_Tagging_RF_2018_Mu.root");
  fin_el = new TFile("Vcb_Tagging_RF_2018_El.root");
  ...
  for (int i = 0; i < list_dir_mu->GetEntries(); i++)
  {
    ...
    for (int j = 0; j < list_key->GetEntries(); j++)
    {
      ...
      if (key_name.Contains("Ratio_"))
      {
        ...
        {
          ratio_mu = (TH1D *)fin_mu->Get(histo_name);
          ratio_el = (TH1D *)fin_el->Get(histo_name);

          ratio_mu->SetBit(TH1::kIsAverage);
          ratio_el->SetBit(TH1::kIsAverage);

          ratio_averaged = (TH1D *)ratio_mu->Clone();
        }
...
void Tagging_RF::Fill_Histo_Validation_MC(const int &sample_index)
{
  ...
  float b_rf = Get_Tagging_RF(vec_short_name_mc[sample_index], "B_Tag_Nominal", n_jets, ht);
  float c_rf = Get_Tagging_RF(vec_short_name_mc[sample_index], "C_Tag_Nominal", n_pv);
...
void Tagging_RF::Setup_Histo()
{
  ...
  for (int i = 0; i < n_sample_merge_mc; i++)
  {
    ...
    histo_mc_before_c[i] = new TH1D(histo_name, histo_name, bin_npv.size() - 1, bin_npv.data());
    ...
    for (int j = 0; j < n_syst_c; j++)
    {
      ...
      histo_mc_after_c[i][j] = new TH1D(histo_name, histo_name, bin_npv.size() - 1, bin_npv.data());
    }
  }
