...
Tagging_RF::Tagging_RF(const TString &a_era, const TString &a_mode, const TString &a_channel, const TString &a_tagger, const int &a_index_tree_type, const int &a_last_index_tree_type, const TString &a_extension) : samples(a_era, a_channel, "Vcb_Tagging_RF")
{
    ...
    // for (int j = 0; j < 3; j++)
    {
      ...
      for (int k = 0; k < n_scores; k++)
      {
        ...
        for (int l = 0; l < 3; l++)
        {
          ...
          TString histo_name = "Closure_D_" + sample_to_draw + "_" + syst_name + "_Leading_Jet_" + score_type + "_" + to_string(l);
          cout << histo_name << endl;

          histo_closure[l] = (TH1D *)fin->Get("D/" + sample_to_draw + "/" + histo_name);
        }
...
void Tagging_RF::Fill_Histo_MC(const int &region_index, const TString &sample_name, const TString &tree_type)
{
  ...
  if (chk_jes_breakdown == true)
  {
    ...
    {
      histo_mc_before_c[region_index][sample_index][27]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][27]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_down);
    }

    else if (tree_type == "JetEnAbsoluteUp")
    {
      histo_mc_before_c[region_index][sample_index][28]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][28]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_up);
    }

    else if (tree_type == "JetEnBBEC1Down")
    {
      histo_mc_before_c[region_index][sample_index][29]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][29]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_down);
    }

    else if (tree_type == "JetEnBBEC1Up")
    {
      histo_mc_before_c[region_index][sample_index][30]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][30]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_up);
    }

    else if (tree_type == "JetEnEC2Down")
    {
      histo_mc_before_c[region_index][sample_index][31]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][31]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_down);
    }

    else if (tree_type == "JetEnEC2Up")
    {
      histo_mc_before_c[region_index][sample_index][32]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][32]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_up);
    }

    else if (tree_type == "JetEnFlavorQCDDown")
    {
      histo_mc_before_c[region_index][sample_index][33]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][33]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_down);
    }

    else if (tree_type == "JetEnFlavorQCDUp")
    {
      histo_mc_before_c[region_index][sample_index][34]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][34]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_up);
    }

    else if (tree_type == "JetEnHFDown")
    {
      histo_mc_before_c[region_index][sample_index][35]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][35]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_down);
    }

    else if (tree_type == "JetEnHFUp")
    {
      histo_mc_before_c[region_index][sample_index][36]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][36]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_up);
    }

    else if (tree_type == "JetEnRelativeBalDown")
    {
      histo_mc_before_c[region_index][sample_index][37]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][37]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_down);
    }

    else if (tree_type == "JetEnRelativeBalUp")
    {
      histo_mc_before_c[region_index][sample_index][38]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][38]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_up);
    }

    else if (tree_type == "JetEnAbsolute2016Down" || tree_type == "JetEnAbsolute2017Down" || tree_type == "JetEnAbsolute2018Down")
    {
      histo_mc_before_c[region_index][sample_index][39]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][39]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_down);
    }

    else if (tree_type == "JetEnAbsolute2016Up" || tree_type == "JetEnAbsolute2017Up" || tree_type == "JetEnAbsolute2018Up")
    {
      histo_mc_before_c[region_index][sample_index][40]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][40]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_up);
    }

    else if (tree_type == "JetEnBBEC12016Down" || tree_type == "JetEnBBEC12017Down" || tree_type == "JetEnBBEC12018Down")
    {
      histo_mc_before_c[region_index][sample_index][41]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][41]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_down);
    }

    else if (tree_type == "JetEnBBEC12016Up" || tree_type == "JetEnBBEC12017Up" || tree_type == "JetEnBBEC12018Up")
    {
      histo_mc_before_c[region_index][sample_index][42]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][42]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_up);
    }

    else if (tree_type == "JetEnEC22016Down" || tree_type == "JetEnEC22017Down" || tree_type == "JetEnEC22018Down")
    {
      histo_mc_before_c[region_index][sample_index][43]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][43]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_down);
    }

    else if (tree_type == "JetEnEC22016Up" || tree_type == "JetEnEC22017Up" || tree_type == "JetEnEC22018Up")
    {
      histo_mc_before_c[region_index][sample_index][44]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][44]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_up);
    }

    else if (tree_type == "JetEnHF2016Down" || tree_type == "JetEnHF2017Down" || tree_type == "JetEnHF2018Down")
    {
      histo_mc_before_c[region_index][sample_index][45]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][45]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_down);
    }

    else if (tree_type == "JetEnHF2016Up" || tree_type == "JetEnHF2017Up" || tree_type == "JetEnHF2018Up")
    {
      histo_mc_before_c[region_index][sample_index][46]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][46]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_up);
    }
    ...
    {
      histo_mc_before_c[region_index][sample_index][47]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][47]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_down);
    }

    else if (tree_type == "JetEnRelativeSample2016Up" || tree_type == "JetEnRelativeSample2017Up" || tree_type == "JetEnRelativeSample2018Up")
    {
      histo_mc_before_c[region_index][sample_index][48]->Fill(n_pileup, ht, weight);
      histo_mc_after_c[region_index][sample_index][48]->Fill(n_pileup, ht, weight * weight_c_tag_jes_total_up);
    }
  } // if (chk_jes_breakdown == true)
...
void Tagging_RF::Fill_Histo_Validation_MC_B_Tagger(const int &region_index, const TString &sample_name, const TString &tree_type)
{
  ...
  for (int i = 0; i < n_syst_b; i++)
  {
    ...
    for (int j = 0; j < 3; j++)
    {
      ...
      histo_closure_n_pileup[region_index][sample_index][i][j]->Fill(n_pileup, weight);
    ...
    for (int j = 0; j < n_sample_merge_mc; j++)
    {
      ...
      for (int k = 0; k < n_syst; k++)
      {
        ...
        for (int l = 0; l < 3; l++)
        {
          ...
          histo_name = histo_name_base + "_N_PILEUP_" + to_string(l);
          histo_closure_n_pileup[i][j][k][l] = new TH1D(histo_name, histo_name, bin_npv.size() - 1, bin_npv.data());
