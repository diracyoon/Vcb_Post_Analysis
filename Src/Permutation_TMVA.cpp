...
Permutation_TMVA::Permutation_TMVA(const TString& a_era, const TString& a_channel, const int& a_n_jet, const bool& a_chk_pre_cut, const bool& a_chk_final_kin)
{
  ...
  TString fout_name;
  if(chk_pre_cut) fout_name = Form("Vcb_PreCut_TTLJ_WtoCB_%dJets.root", n_jet);
  else if(chk_final_kin) fout_name = Form("Vcb_Permutation_TTLJ_WtoCB_%dJets.root", n_jet);
  else fout_name = Form("Vcb_PrePermutation_TTLJ_WtoCB_%dJets.root", n_jet);
  ...
  n_train_signal = tree_correct->GetEntries(cut_s)/2;
  n_train_back = tree_wrong->GetEntries(cut_b)/2;
