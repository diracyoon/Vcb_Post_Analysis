...
histo_nominal = fin.Get('%s/Nominal/%s/Template_MVA_Score'%(region,sample))
...
print('%s/%s_Smoothed/%s/Template_MVA_Score'%(region,syst,sample))
...
histo_nominal.GetXaxis().SetTitle('Template MVA Score')
histo_nominal.Draw('')
...
legend = ROOT.TLegend()
legend.AddEntry(histo_nominal, "Nominal", "lp")
legend.AddEntry(histo_raw, "Syst_Raw", "lp")
legend.AddEntry(histo_smoothed, "Smoothed", "lp")
legend.Draw("SAME")
